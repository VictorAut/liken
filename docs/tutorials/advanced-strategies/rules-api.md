...
??? question "Yet Another API...?"
    The **Rules API** defines advanced usage, and offers tooling that is unavailable to the **Sequential** and **Dict API**s. 
...
Let's first use the **Rules API** to replicate the complex strategy we saw as an example earlier in the tutorial on applying strategies as dictionaries in the [**Dict API** tutorial](../applying-strategies/dict-api.md). To do so we will introduce the `rules` sub-package. Let's jump straight to an example:
...
!!! note
    The **Rules API**, just like **Dict API**, also enforces the use of a single call to `apply()`. The emphasis is still on you to construct stragegies that appropriately model the deduplication (or canonicalization) needs of your DataFrame.


## Collections of `on`

`Rules` don't accept length-2 tuples. They *need* `on`. The naming might give you a clue as to why. It's because your "rule" will be applied "on" a column. The nature of these rules will be explained in the next tutorial.
