...
In your [First Steps](../first-steps.md#the-simplest-example) you found out how to use the `Dedupe` class with exact deduplication — in fact it was *the* [exact](../../reference/liken.md#liken.exact) strategy. It came bundled with `Dedupe` when you called `drop_duplicates()` with no other strategies.
...
Throughout **Liken** when you want to use a strategy, you `apply` it.
...
The Sequential API will get you quite far and for many use cases it is sufficient for the casual user of **Liken**. For example, it is an excellent way to add functionality to what would otherwise remain a Pandas data pipeline. However, in this tutorial you may have noticed that building a "complex" strategy with multiple stratgies applied to *different* columns would be quite verbose using the **Sequential API**. In the next tutorial we'll explore the use of something more flexible.
