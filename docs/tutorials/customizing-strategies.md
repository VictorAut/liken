...
- This is actually a *generator*, which is more efficiently consumed by **Enlace**. You can choose to build a list and return that, but it's less efficient.
...
Having defined our custom strategy, we're now ready to make is useable by the `Dedupe` class. Right now our function won't ever know what `array` is and nor is it ready to plug into the inner workings of **Enlace**. So, we're going to *register* the function for use:
...
=== "Dict API"
    ...
    ```python {hl_lines="2"}
    dp = Dedupe(df)
    dp.apply({"address": str_same_len(min_len=12)})
...
=== "Rules API"
    ...
    ```python {hl_lines="2"}
    dp = Dedupe(df)
    dp.apply(Rules("address", on(str_same_len(min_len=12))))
    df = dp.drop_duplicates()
    ```
...
!!! success "You learnt:"
    ...
    - You can make use of your function by using the `@register` decorator from the `enlace.custom` sub-package.
    - You can apply your strategy just like you would any other **Enlace** strategy. 