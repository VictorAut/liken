...
SUPPORTED_DFS = pd.DataFrame | pl.DataFrame | spark.DataFrame | mpd.DataFrame | RayFrame
...
INVALID_DF: Final[str] = (
    INVALID
    + "df must be istance of Pandas, Polars, Modin, Ray or Spark DataFrames, got {}"
)
INVALID_SPARK: Final[str] = (
    INVALID + "spark_session arg must be provided for a spark dataframe"
)
INVALID_KEEP: Final[str] = INVALID + "keep arg must be one of 'first' or 'last', got {}"
INVALID_DEDUPER: Final[str] = (
    INVALID + "deduper must be instance of BaseDeduper, got {}"
)
INVALID_PREPROCESSOR: Final[str] = (
    INVALID + "preprocessor must be instance of Preprocessor, got {}"
)
...
INVALID_COLUMNS_REPEATED: Final[str] = (
    INVALID + "columns labels cannot be repeated. Repeated labels: '{}'"
)
INVALID_COLUMNS_NOT_NONE: Final[str] = (
    INVALID
    + "columns must be None when using the dict API, as they have been defined as dictionary keys."
)

# collection errors

INVALID_DICT_KEY_MSG: Final[str] = (
    "Invalid type for dict key type: expected str or tuple, got '{}'"
)
INVALID_DICT_VALUE_MSG: Final[str] = (
    "Invalid type for dict value: expected list, tuple or 'BaseDeduper', got '{}'"
)
...
INVALID_RULE_MEMBER_MSG: Final[str] = (
    "Invalid Pipeline element at index {} is not an instance of Col, got '{}'"
)
INVALID_FALLBACK_MSG: Final[str] = (
    "Invalid deduper: Expected a 'BaseDeduper', a dict or 'Pipeline', got '{}'"
)

# collection warnings

WARN_DICT_REPLACES_SEQUENCE_MSG: Final[str] = (
    "Replacing previously added sequence deduper with a dict deduper"
)
